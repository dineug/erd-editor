...
| `intellij-webview`'s `build:webview` | that same plugin repo — it emits into `../../../erd-editor-intellij-plugin/src/main/resources/assets` with `emptyOutDir`, and is `cache: false` because the cache cannot restore files outside this tree. The plugin repo also vendors this one as a submodule, one directory deeper, so `resolveWebviewOutDir` detects the layout by looking for `plugin.xml` in the parent; `ERD_EDITOR_ASSETS_DIR` overrides both |

...
| `tools/vite/library-config.ts` | Typed factory for the six standard library builds, the `workers: true` half a library that spawns one adds, and the shared task builder used by all eight library-mode packages |
...
| `scripts/check-bundle-size.mjs` | The `pnpm size` gate — concatenates every `packages/erd-editor/dist/**/*.js` in path order, gzips that at level 9 and checks it against `packages/erd-editor/.size-baseline.json`. It reads a build artifact, so it runs after `pnpm build` in CI and never inside `pnpm check` |
...
Build order is derived from workspace dependencies: the leaves are `r-html`, `vite-plugin-r-html`, `schema-sql-parser`, `erd-editor-shiki-worker`, `erd-editor-schema` and `vscode-bridge`, and `erd-editor` sits on the first five. `app`, `intellij-webview` and `vscode-webview` depend on `erd-editor` directly, the two webviews also through `vscode-replication-store-worker`; `vscode-extension` reaches it only through `vscode-webview`, which is the longest chain — `vuerd-vscode` → `vscode-webview` → `vscode-replication-store-worker` → `erd-editor` → `erd-editor-schema`. `intellij-plugin` sits outside that graph: a Gradle project whose only pnpm presence is a `private` package.json, fed by `intellij-webview`'s build output.
...
| `packages/vscode-replication-store-worker` | `@dineug/erd-editor-vscode-replication-store-worker` | Worker | headless document replica spawned by both IDE webviews |
...
- **`erd-editor` and `erd-editor-shiki-worker` publish over external dependencies.** Each `dependencies` list is exactly the third-party packages its shipped `src/` imports (`erd-editor` adds `stylis` for the inlined `r-html`); the lib builds externalize that list and inline the private workspace libraries. Their SharedWorkers, like the private `vscode-replication-store-worker`'s worker (`defineLibraryConfig(..., { workers: true })`), are files under `dist/workers/`, constructed as `new SharedWorker(new URL('./workers/x.js', import.meta.url), { type: 'module' })` — `tools/vite/worker-url.ts` rewrites Vite's page-shaped URL into that spelling, the one webpack, Rspack and Vite all bundle from inside a dependency — and `vscode-webview` alone turns them back into inline workers through `tools/vite/inline-worker.ts`, because that host serves its scripts from an origin other than the document's and a worker is the one resource a browser refuses across that line; `intellij-webview` serves everything from one origin and loads them from their URLs. There is no CDN build, so a consumer needs a bundler, and there is deliberately no `sideEffects` field: `packages/erd-editor/AGENTS.md` says why.
...
- **One bundler.** Eight libraries build in Vite library mode with `vite-plugin-dts`; `app`, `intellij-webview`, `vscode-webview` and `vscode-extension` build an entry. Match the neighbouring package.
- **`@/*` → `<package>/src/*`** is in all 12 TypeScript packages' `tsconfig.json` `paths`. The six standard library builds receive the matching Vite alias from `tools/vite/library-config.ts`; bespoke build and test configs still declare it locally.
...
- `pnpm test` = `vp run -r test`. Eight packages define a `test` task — `r-html`, `schema-sql-parser`, `erd-editor-schema`, `erd-editor`, `vscode-bridge`, `vite-plugin-r-html`, `app`, `vscode-extension`; the other five no-op. Each runs `tsc --noEmit` before Vitest.
- Seven Vitest configs use `include: ['src/**/*.test.ts']`. `erd-editor` is the exception: it declares two `projects` — `unit` (happy-dom, `src/**/*.test.{ts,tsx}`) and `browser` (a real headless Chromium through `@vitest/browser-playwright`, `src/**/*.browser.test.{ts,tsx}`), because its ERD scene is a Konva canvas that means nothing without one. The `.browser.` infix is the only thing that routes a spec to the second project. **`pnpm test` therefore needs a browser binary now**, which is why the `ci` job installs Chromium before running it. A test outside `src/` is never collected, and all eight configs have a v8 coverage block at `perFile` 80%. Those thresholds gate `pnpm --filter <pkg> test:coverage` only; `pnpm test` and CI do not enforce them.
...
- Barrel `index.ts` per feature directory; a package's root `src/index.ts` is the public surface — named re-exports in most, blanket `export *` in `r-html` and `vscode-bridge`.
- The seven ESM library packages are `"type": "module"` with an `exports` map of `types` + `default`, and point `main` / `module` / `types` at `dist/`.
...
- **Vite+ 0.2.9** — tasks, lint, format, test and commit hooks in one toolchain; `tools/vite/worker-url.ts` and `tools/vite/inline-worker.ts` are the two plugins its worker handling needs for a library and for the VSCode webview, both unit-tested from `tools/vite-config.test.ts`; `vp toolchain` is what prints the bundled tool versions. The catalog aliases `vite` to `@voidzero-dev/vite-plus-core`, so there is no `node_modules/.bin/vite` to invoke.

...
`@dineug/erd-editor-monorepo` is a pnpm + Vite+ workspace for an Entity-Relationship Diagram editor shipped five ways: erd-editor.io, a VSCode extension, an IntelliJ plugin, an Obsidian plugin and the `<erd-editor>` custom element on npm. The framework-free editor core is JSX compiled to the in-house `@dineug/r-html` tagged templates. Its store's actions carry a Lamport-style clock and merge through an LWW register set — the one mechanism behind collaboration, cross-tab sync and undo/redo. The same mechanism lets `@dineug/erd-editor-mcp`, a stdio MCP server also on npm, put a coding agent into a VS Code or Obsidian editing session as one more collaborator.
...
| `tools/vite/library-config.ts` | `defineLibraryConfig` (the whole config of nine library packages, one `src/index.ts` build each; `node: true`, which `agent-hub-host` sets, targets `node22` and keeps every node builtin external, bare and `node:`-prefixed), `createLibraryTasks` (task contract of all ten library packages; `erd-editor` uses it alone) |
| `tools/vite/package-metadata.ts` | Task inputs derived from tsconfig files and manifests; `createExternal` (dependencies and peers, plus the builtins it is given) |
...
| `scripts/check-task-inputs.mjs` | Recomputes library tasks; matches bespoke tasks' `.d.ts` globs to declared deps; pins the 10 / 9 library-config counts a new library must update |
...
| Directory | Purpose |
| --- | --- |
| `packages/` | The 17 workspace packages, each with its own `AGENTS.md` |
...
Build order follows workspace dependencies; the longest chain is `vuerd-vscode` → `vscode-webview` → `webview-client` → `replication-store-worker` → `erd-editor` → `erd-editor-schema`. `mcp-server` joins only its tail (`mcp-server` → `erd-editor` → `erd-editor-schema`; it also names `erd-editor-schema` and `r-html` itself, for the parser and the action types), and its other dependency, `agent-hub`, depends on no workspace package; `effect` is its one peer. `agent-hub-host` depends on `agent-hub` alone, with `effect` and `@effect/platform-node` as its peers, and `vuerd-vscode` and `obsidian-plugin` inline both hub packages. `intellij-plugin` is Gradle, outside the graph, fed by `intellij-webview`'s build. `obsidian-plugin` bundles `erd-editor`, `replication-store-worker`, `webview-bridge`, `agent-hub-host`, `agent-hub`, `effect`, `@effect/platform-node` and `monkey-around` into one `main.js`; only `obsidian` and node builtins stay imports.
...
| `agent-hub` | `@dineug/erd-editor-agent-hub` | the editor window ↔ coding-agent hub protocol as an effect `Schema` spec, `effect` its one peer dependency: messages, lock file, JSON lines framing, path authorization, discovery |
| `agent-hub-host` | `@dineug/erd-editor-agent-hub-host` | the editor window's side of that hub on Node, which `vuerd-vscode` and `obsidian-plugin` build their document sides on: the listener, serving a connection, path authorization, the lock file, the hub's lifecycle over a host's `HubHost` / `HubDocuments`, the join-window rules; `effect` and `@effect/platform-node` its peers |
...
| `vscode-extension` | `vuerd-vscode` | VSCode extension host, published, and the document side of the hub coding agents join, on `agent-hub-host` |
...
| `obsidian-plugin` | `@dineug/erd-editor-obsidian-plugin` | Obsidian plugin, whose `dist/` is the plugin folder, and a vault window's document side of the hub coding agents join, on `agent-hub-host`; released from `dineug/erd-editor-obsidian-plugin`, which carries this repository as a submodule |
...
| `mcp-server` | `@dineug/erd-editor-mcp` | stdio MCP server for coding agents, published: effect's `McpServer` over stdio, one tool per editing op, live through a VS Code or Obsidian window's hub or headless on the file; one ESM file with nothing external but node builtins |
...
- **Library builds.** Eight private libraries (`r-html`, `vite-plugin-r-html`, `schema-sql-parser`, `erd-editor-schema`, `webview-bridge`, `webview-client`, `agent-hub`, `agent-hub-host`) build `minify: false` + `preserveModules: true` with `sideEffects: false`, so the consumer prunes per file and minifies once; `agent-hub-host` alone adds `node: true`, since it imports node builtins, which a browser build resolves to an empty stub with only a warning. `erd-editor` keeps chunks and no `sideEffects` field (its `AGENTS.md` says why).
- **Externals decide what ships.** `createExternal` keeps `dependencies` + `peerDependencies` as bare imports and inlines the rest. `erd-editor` lists the private libraries as devDependencies so they inline; moving one into `dependencies` ships an import of a package not on npm. Two private libraries have peers: `agent-hub` names `effect`, `agent-hub-host` names `effect` and `@effect/platform-node`, and `agent-hub-host` names `agent-hub` in `dependencies`. Their `dist/*.js` and `.d.ts` keep those imports bare, and each consumer (`mcp-server`, `vuerd-vscode`, `obsidian-plugin`) lists them as devDependencies, effect's from the same `catalog:` pin, and inlines the one copy it resolves (`ssr.noExternal: true` in the Node bundles), shared with its own effect code; two copies of `agent-hub` would make two `HubRequestError` classes, and every refusal would answer `internal`. A consumer program that type-checks those `.d.ts` files needs `skipLibCheck`, which `tsconfig.app.json` and the extension's `tsconfig.json` set: effect's own declarations name types lib ES2022 lacks (`TextDecoderOptions`); `agent-hub-host`'s also need `@types/node`.
- **Workers.** `erd-editor`'s four SharedWorkers and the replica Worker ship as `dist/workers/*.js`, spawned from `new URL('./workers/x.js', import.meta.url)` — the spelling `worker-url.ts` writes, which webpack, Rspack and Vite all bundle from a dependency. `vscode-webview` rebuilds them as same-origin blobs (`same-origin-worker.ts`), `intellij-webview` loads them by URL, the UMD build and `obsidian-plugin` inline them (`inline-worker.ts`).
...
- `pnpm test` = `vp run -r test` over the thirteen packages with a `vitest.config.*` (a library's `test` task exists because of that file), each `tsc --noEmit` then Vitest, imported as `vite-plus/test`.
...
- v8 coverage at `perFile` 80% on all four metrics gates `test:coverage`, and `pnpm test` measures none. CI runs `pnpm -r --no-bail test:coverage`, so a file under 80% in any of the thirteen packages fails it (`obsidian-plugin` measures `src/hub/`, `src/settings.ts` and `src/tabSave.ts`; the rest runs only inside Obsidian, under the smoke). Cover a gap with a test; the one hint in `src/` is `/* v8 ignore next -- @preserve */` on `erd-editor`'s `if (import.meta.hot)` blocks.
...
- `pnpm --filter @dineug/erd-editor-obsidian-plugin smoke`, outside `pnpm test` and CI: the built plugin in a real Obsidian (`/Applications/Obsidian.app`, or `OBSIDIAN_BIN`; `OBSIDIAN_ASAR` names an `obsidian-<version>.asar` from Obsidian's user data folder, which loads over the bundled app as an update does) on a throwaway vault and user data directory, driven over CDP, and the coding-agent hub driven by the built MCP server over stdio, so build `@dineug/erd-editor-mcp` first. Its window's lock and socket go to the real `~/.erd-editor/ide` while it runs; it checks that quitting removed them and deletes only its own renderer pid's files, except under `SMOKE_KEEP=1`, which leaves Obsidian running with both in place.
...
- Toolchain: **Vite+ 0.2.9** (`vp`), **pnpm 10.34.3**, **Node 22.23.2** (`.nvmrc` = `.node-version`; the floor a package declares is `engines.node` `>=22.12.0`, in the root, `mcp-server`, `agent-hub` and `agent-hub-host`, and the extension's is VS Code 1.101, which runs Node 22.15), **TypeScript 7.0.2** (+ `@typescript/typescript6` 6.0.2), **Vitest 4.1.10**, **Playwright `^1.62.1`** everywhere, `@vscode/test-cli`, `eslint-plugin-simple-import-sort`, commitlint 20.
...
| `dineug/erd-editor-obsidian-plugin` | this repository as a submodule: it releases `packages/obsidian-plugin/dist/` as the assets of a GitHub release tagged with the manifest version, and keeps a copy of the package's `manifest.json` at its root, where Obsidian looks for the latest version. The manifest `id` (`erd-editor`) cannot change once the plugin is in the community directory. Its `README.md` is the plugin's listing and carries the disclosure Obsidian's developer policies require for files outside the vault (the hub's lock and socket under `~/.erd-editor/ide`), so a change to what the hub writes outside the vault, or a network request, updates it |

...
describe('cross argument rules', () => {
  ...
  const refused = (name: string, args: Record<string, unknown>) => {
    let thrown: unknown;
    try {
    ...
    expect((thrown as ToolError).code, name).toBe(ToolErrorCode.invalidArgs);
    return (thrown as ToolError).message;
  };

  it('refuses an index column moved onto itself', () => {
    expect(
      refused('erd_move_index_column', {
        indexId: SEED.index,
        indexColumnId: SEED.indexColumn,
  ...
  it('refuses a memo smaller than the resize handles allow', () => {
    for (const [width, height] of [
      ...
      expect(
        refused('erd_resize_memo', { memoId: SEED.memo, width, height })
      ).toBe(
  ...
  it('refuses to move a column onto itself', () => {
    const own = createSeededPeer();
    let thrown: unknown;

    try {
      runTool(own, 'erd_move_column', {
        tableId: SEED.orders,
    ...
    expect(thrown).toBeInstanceOf(ToolError);
    expect((thrown as ToolError).code).toBe(ToolErrorCode.invalidArgs);

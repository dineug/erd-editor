...
`smoke` copies `dist/` into a throwaway vault, starts Obsidian with its own `--user-data-dir` (your vaults are untouched), enables the plugin over CDP and checks opening every extension, saving through `Alt+N`, two tabs of one file editing in step, closing without an edit, closing before the timed save, closing two tabs of one file, the unreadable-file fallback, an outside change during an edit, `openFile(null)`, the create command, the containment override and, last, quitting before the timed save. `OBSIDIAN_BIN` points it at another install. To try a build in a vault of your own, link `dist/` in as `<vault>/.obsidian/plugins/erd-editor`.

## Dependencies

### Internal

All devDependencies, bundled: `@dineug/erd-editor` (the element), `@dineug/erd-editor-replication-store-worker` (the replica), `@dineug/erd-editor-webview-bridge` (the replica's commands).

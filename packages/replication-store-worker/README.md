...
`@dineug/erd-editor-replication-store-worker` keeps a second copy of the open document inside
...
```ts
import { Bridge, hostSaveValueCommand } from '@dineug/erd-editor-webview-bridge';
...
```sh
pnpm exec vp run --filter @dineug/erd-editor-replication-store-worker --fail-if-no-match build
```

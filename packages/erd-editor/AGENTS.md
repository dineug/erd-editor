...
- `generator-code/sqlalchemy.ts` is the one generator that emits an import header and `class Base(DeclarativeBase)`, in the per-table path as well as the whole-document one — Python has no implicit namespace, so a bare class body is a `NameError`; `typeorm.ts` names its decorators the way `jpa.ts` names its annotations, unimported. Four things `sqlalchemy.ts` renders faithfully rather than repairing, and `typeorm.ts` shares all four: the cross-class reference — a SQLAlchemy forward-reference string, a TypeORM `() => Target` — so a per-table snippet names classes it does not contain, and a table with no primary key becomes a class the mapper rejects; the other two follow `createSchemaSQL`, which declares the same collision — two tables of one name keep that table name (only their class names are deduplicated), and an unnamed column reaches `mapped_column("")` / `@Column({ name: "" })`. `typeorm.ts` adds two of its own: an auto-increment primary key whose type is outside `@PrimaryGeneratedColumn`'s numeric union loses that type, and its one-to-one and many-to-one properties are typed `Relation<T>` rather than `T` — every class lands in one module, so an annotation can name a class declared below it, and under `emitDecoratorMetadata` the `design:type` is read eagerly.

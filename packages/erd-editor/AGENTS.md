...
- `generator-code/sqlalchemy.ts` is the one generator that emits an import header and `class Base(DeclarativeBase)`, in the per-table path as well as the whole-document one — Python has no implicit namespace, so a bare class body is a `NameError`. Four things it renders faithfully rather than repairing. Two `jpa.ts` shares: `relationship()` targets are forward-reference strings, so a per-table snippet names classes it does not contain, and a table with no primary key becomes a class the SQLAlchemy mapper rejects. Two follow `createSchemaSQL`, which declares the same collision: two tables of one name keep that `__tablename__` (only their class names are deduplicated, since a repeated `class` statement is not an error Python reports), and an unnamed column reaches `mapped_column("")`.

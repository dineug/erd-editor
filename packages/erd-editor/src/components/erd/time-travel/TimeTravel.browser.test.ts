// P3-27: the preview is a konva scene, so this spec runs in the browser
// project, where a Stage can be built and its nodes counted.
...
/**
 * The preview is drawn on two stages now, so the tables are konva nodes rather
 * than elements. Both are counted, which is what the dom query this replaces
 * did when it reached the canvas copy and the minimap copy of each table.
 */
const tableCount = () => {
  ...
  return (
    (stages.canvas?.find('.table').length ?? 0) +
    (stages.minimap?.find('.minimap-table').length ?? 0)
  );
};

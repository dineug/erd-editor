...
describe('Table', () => {
  ...
  it('reorders the columns when one is dragged over another', async () => {
    ...
    dragEvent(columnRows().find(el => el.dataset.id === secondId)!, 'dragover');
    await wait(120);
    await flush();
  ...
  it('keeps a ghost row for a column dragged into another table', async () => {
    ...
    dragEvent(rowsOf(aEl).find(el => el.dataset.id === c1)!, 'dragstart');
    await flush();

    dragEvent(rowsOf(bEl)[0], 'dragenter');
    await flush();

    dragEvent(rowsOf(bEl).find(el => el.dataset.id === d1)!, 'dragover');
    await wait(120);
    await flush();

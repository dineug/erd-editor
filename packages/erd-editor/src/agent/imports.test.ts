...
// AC-P12, source half: the agent entry runs in a Node process with no DOM and
// no bundler, so what it reaches must neither touch a browser global nor name
// a package the MCP bundle cannot inline or resolve.
...
const ENTRY = join(SOURCE_ROOT, 'agent', 'index.ts');
...
describe('agent source graph (AC-P12)', () => {
  const graph = walk(ENTRY);

  it('starts at the agent entry and reaches the stores it wraps', () => {
    expect(graph.files).toContain('agent/index.ts');
    expect(graph.files).toContain('engine/rx-store.ts');
    expect(graph.files).toContain('engine/shared-store.ts');
  });
  ...
  it('keeps every workspace library it names out of dependencies, so it inlines', () => {
    const named = inlinedWorkspaceLibraries.filter(name =>
      graph.bare.has(name)
    );
...
await import('./f');`.matchAll(IMPORT_FORM),
  ...
  it('fails loudly on an import it cannot resolve', () => {
    expect(() => resolveSource(join(SOURCE_ROOT, 'agent', 'missing'))).toThrow(
      'Unresolved import'
    );
  });
});

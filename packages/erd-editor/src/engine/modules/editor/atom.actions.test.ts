...
describe('editor.changeViewport', () => {
  ...
  /**
   * A window that grows shortens the travel the origin is allowed. Leaving it
   * outside paints a band of nothing along two edges until the next scroll
   * gesture happens to clamp it, which is a repaint the user has to ask for.
   */
  it('pulls an origin left outside the widened range back into it', () => {
    store.dispatchSync(changeViewportAction({ width: 900, height: 700 }));
    store.dispatchSync(
      scrollToAction({ originX: -1_000_000, originY: -1_000_000 })
    );
    expect(store.state.settings.originX).toBe(900 - 2000);
    expect(store.state.settings.originY).toBe(700 - 2000);

    store.dispatchSync(changeViewportAction({ width: 1440, height: 900 }));

    expect(store.state.settings.originX).toBe(1440 - 2000);
    expect(store.state.settings.originY).toBe(900 - 2000);
  });

  it('leaves an origin the narrowed range still holds exactly where it was', () => {
    store.dispatchSync(changeViewportAction({ width: 1440, height: 900 }));
    store.dispatchSync(scrollToAction({ originX: -100, originY: -200 }));
   ...
   * closes toward the middle of the screen as the zoom falls, so the origin
  ...
  it('pulls a shrunk canvas back to the end of the travel it still has', () => {
    ...
    // The far end of a travel that closes in by the zoom rather than the canvas
    // box's own end, read off the range the reducer clamps with.
    const { left, top } = getScrollRanges(
      store.state.settings,
      store.state.editor.viewport
    );
    expect(store.state.settings.originX).toBe(round(left.min, 4));
    expect(store.state.settings.originY).toBe(round(top.min, 4));
  });
});

/** A document that names its zoom and its origin and nothing else. */
const documentAt = (zoomLevel: number, originX: number, originY: number) =>
  JSON.stringify({
    version: '3.0.0',
...
describe('editor.loadJson / initialLoadJson', () => {
  /**
   * A file can name an origin no zoom below 1 can hold, and nothing else on the
  ...
  it.each([
    ...
    'pulls an origin %s carries into the travel its zoom allows',
    (_, load) => {
      store.dispatchSync(changeViewportAction({ width: 1440, height: 900 }));
      const { left, top } = getScrollRanges(
        { width: 2000, height: 2000, zoomLevel: 0.3 },
        { width: 1440, height: 900 }
      );

      store.dispatchSync(
        load({ value: documentAt(0.3, 1_000_000, 1_000_000) })
      );

      // The near end of the travel, written out: the half screen a zoom of 1
      // shows, shrunk by the zoom, between the screen's middle and its edge.
      expect(left.max).toBe((1440 * (1 - 0.3)) / 2);
      expect(top.max).toBe((900 * (1 - 0.3)) / 2);
      expect(store.state.settings.zoomLevel).toBe(0.3);
      expect(store.state.settings.originX).toBe(round(left.max, 4));
      expect(store.state.settings.originY).toBe(round(top.max, 4));
    }
  );
  ...
  it('leaves an origin the travel already holds exactly where the file put it', () => {
    ...
    store.dispatchSync(loadJsonAction({ value: documentAt(0.3, 400, 300) }));
   ...
   * an origin of 700 on each axis, past the travel the screen allows: the pull
   * lands it on the end while the legacy pair stays exactly as written.
   */
  it('migrates a legacy document, then pulls the origin into range', () => {
    store.dispatchSync(changeViewportAction({ width: 1440, height: 900 }));
    const { left, top } = getScrollRanges(
      { width: 2000, height: 2000, zoomLevel: 0.3 },
      { width: 1440, height: 900 }
    ...
    store.dispatchSync(loadJsonAction({ value: legacyDocumentAt(0.3, 0, 0) }));

    expect(origin.originX).toBeGreaterThan(left.max);
    expect(origin.originY).toBeGreaterThan(top.max);
    expect(store.state.settings.originX).toBe(round(left.max, 4));
    expect(store.state.settings.originY).toBe(round(top.max, 4));
  ...
  it('keeps a migrated origin the travel holds, and the legacy pair beside it', () => {
    store.dispatchSync(changeViewportAction({ width: 1440, height: 900 }));

    store.dispatchSync(
      loadJsonAction({ value: legacyDocumentAt(0.3, -300, -400) })
    );

    expect(store.state.settings).toMatchObject(migrated(0.3, -300, -400));
   ...
   * file's own origin is kept and changeViewport clamps it once there is a
   * screen. Clamping against a screen of nothing would move it twice.
   */
  it('keeps the origin until a screen exists, then lands it in range', () => {

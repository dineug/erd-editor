...
| `src/helper.ts` | `assign`, `validString`, `validNumber`, `propOr`, `getDefaultEntityMeta` — every parser is built from these |
| `vite.config.ts` | Library build (`BROWSER_TARGET`, ES-only, `vite-plugin-dts`) **and** the `build` / `test` tasks |
...
- Parsers must never throw on malformed input: they take `any`, validate per field, and fall back to the factory default. The editor relies on `schemaV3Parser({})` producing a complete empty document.

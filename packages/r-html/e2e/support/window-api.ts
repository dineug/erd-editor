/**
 * The contract between `e2e/fixture/main.ts` (which implements it in the page)
 * and `e2e/support/CssPage.ts` (which calls it over the CDP bridge).
 *
 * Types only — no import that could pull Playwright into the page bundle, and
 * nothing here is emitted. Both sides import this file, so a rename breaks the
 * typecheck instead of a run.
 */
...
/**
 * What one `adoptedStyleSheets` **assignment** does to the array it was handed.
 *
 * The spec says the setter copies into the host's own backing list. happy-dom keeps
 * the reference instead, which is the whole reason `adoptInto` spends a `[...sheets]`.
 * Every field is reported rather than reduced to a verdict, so a failure names the
 * half that moved.
 */
...
export interface RHtmlE2E {
  ...
  /**
   * The control for `benchmarkRegister`: the same number of sheets put onto the
   * same mounted hosts through the raw platform call, with no compile, no hash
   * and no `replaceSync` in the measured window. `push` against `reassign` is
   * exactly the difference the append fast path makes, isolated from everything
   * else registration does. Returns elapsed ms; leaves every host's list empty.
   */

...
3. **Live collaboration** — a socket.io room where the _host_ shares its editor action stream. The room
   URL carries `#<roomId>,<secretKey>`; the key never reaches the server.
...
the URL fragment (`location.hash`), which browsers never send to the server. Every payload is
encrypted client-side (`encryptToJson`) and decrypted by peers (`decryptFromJson`), so the relay
...
- **The secret key must stay in the URL fragment.** `LiveCollaborative` parses
  ...
  a path segment, or any request body would leak it to the server and break the E2E guarantee.
...
- `private: true`; the websocket endpoint comes from `import.meta.env.WEBSOCKET_URL`.

### Testing Requirements

- No `test` target. Verification is `pnpm --filter @dineug/erd-editor-app dev` (webpack-dev-server,
  opens automatically) and `pnpm --filter @dineug/erd-editor-app build`.
...
- **Collaboration changes**: start a session in one browser profile and join the `/live#...` URL in
  another; confirm the guest sees updates and that stopping the host raises `HostStopSessionError`
  cleanly. Verify with the network tab that only ciphertext leaves the page.
...
- `socket.io-client` — collaboration transport

...
| `gdrive/specs/gdrive-auth.spec.ts` | The relay popup, the refresh cookie, one renewal between two tabs, sign-out, the state through sign-in, the account switch and the ways out of it, Drive left out, a stray callback link, the CSRF gate |

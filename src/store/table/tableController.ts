  ...
  for (let i = 0; i < state.tables.length; i++) {
    if (state.tables[i].ui.active) {
      store.relationshipStore.commit(
        RelationshipCommit.relationshipRemoveTable,
        {
...
export function tableSelect(
  state: State,
  payload: { table: Table; event: MouseEvent; store: StoreManagement }
) {
  ...
  if (event.ctrlKey) {
    ...
    state.tables.forEach(
      (value: Table) => (value.ui.active = value.id === table.id)
    );
    store.memoStore.commit(MemoCommit.memoSelectAllEnd);
  }
  tableFocusStart(state, { table, store });
  if (store.relationshipStore.state.draw) {
    if (store.relationshipStore.state.draw.start) {
      ...
      store.relationshipStore.commit(
        RelationshipCommit.relationshipDrawStartAdd,
        {
          table,
          store
        }
...
export function tableFocusStart(
  state: State,
  payload: { table: Table; store: StoreManagement }
) {

...
export function memoRemove(
  state: State,
  payload: { memo: Memo; store: StoreManagement }
) {
...
export function memoSelect(
  state: State,
  payload: { memo: Memo; event: MouseEvent; store: StoreManagement }
) {
  ...
  if (event.ctrlKey) {
    ...
    state.memos.forEach(
      (value: Memo) => (value.ui.active = value.id === memo.id)
    );
    store.tableStore.commit(TableCommit.tableSelectAllEnd);
  }
}
...
export function memoMultipleSelect(
  ...
  state.memos.forEach((memo: Memo) => {
    ...
    const centerY =
      memo.ui.top + memo.ui.height / 2 + MEMO_PADDING + MEMO_HEADER;
    memo.ui.active = !!(
      min.x <= centerX &&
      centerX <= max.x &&
      min.y <= centerY &&

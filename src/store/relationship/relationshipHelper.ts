...
function getDrawDirection(draw: RelationshipDraw): Direction {
  ...
  if (draw.start) {
    ...
    let min =
      Math.abs(start.bottom.x - draw.end.x) +
      Math.abs(start.bottom.y - draw.end.y);
    ...
    Object.keys(start)
      ...
      .forEach(key => {
        ...
        const temp =
          Math.abs(start[k].x - draw.end.x) + Math.abs(start[k].y - draw.end.y);
        if (min > temp) {
          min = temp;
          direction = k;
          if (draw.start) {
...
function getDirection(
  ...
  let min =
    Math.abs(start.bottom.x - end.bottom.x) +
    Math.abs(start.bottom.y - end.bottom.y);
    ...
    .forEach(key => {
      Object.keys(end)
        ...
        .forEach(key2 => {
          ...
          const temp =
            Math.abs(start[k].x - end[k2].x) + Math.abs(start[k].y - end[k2].y);
  ...
  endPoints.forEach((endPoint, index) => {
    if (startPoints[index].tableId === endPoint.tableId) {
      ...
      distances.push({
        ...
        distance:
          Math.abs(start.x - endPoint.x) + Math.abs(start.y - endPoint.y)
      });

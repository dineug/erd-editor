    ...
    tables.forEach((table) => {
      ...
      table.columns.forEach((column) => {
        if (column.option.autoIncrement) {
          ...
          stringBuffer.push(`CREATE OR REPLACE TRIGGER ${canvas.databaseName}.${trgName}`)
          stringBuffer.push(`BEFORE INSERT ON ${canvas.databaseName}.${table.name}`)
          stringBuffer.push(`REFERENCING NEW AS NEW FOR EACH ROW`)
          stringBuffer.push(`BEGIN`)
          stringBuffer.push(`\tSELECT ${canvas.databaseName}.${aiName}.NEXTVAL`)

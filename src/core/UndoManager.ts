import { Command, CommandKey } from '@/internal-types/UndoManager';
...
export function createUndoManager(effect: () => void) {
  const undoManager = new UndoManager(effect);

  const undo = () => {
    undoManager.hasUndo && undoManager.undo();
  };
  const redo = () => {
    undoManager.hasRedo && undoManager.redo();
  };
